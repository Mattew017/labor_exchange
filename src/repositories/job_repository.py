...
class JobRepository(IRepositoryAsync):
    ...
    async def retrieve(self, include_relations: bool = False, **kwargs) -> JobModel:
        async with self.session() as session:
            ...
            if include_relations:
                query = query.options(joinedload(Job.user)).options(
                    selectinload(Job.responses)
                )
    ...
    async def update(self, id_: int, job_update_dto: JobUpdateSchema) -> JobModel:
        async with self.session() as session:
            query = select(Job).filter_by(id=id_).limit(1)
            res = await session.execute(query)
            job_from_db = res.scalars().first()
    ...
    async def delete(self, id_: int):
        async with self.session() as session:
            ...
            if job_from_db:
                ...
                await session.commit()
                logger.debug("Deleted job id=%d", id_)
            else:
                raise EntityNotFoundError("Вакансия не найдена")
